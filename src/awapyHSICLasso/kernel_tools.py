...
def kernel_delta_umbalance(X_in_1,X_in_2):
    # by awa
    u_list=np.unique(X_in_1)
    n_1 = X_in_1.shape[1]
    n_2 = X_in_2.shape[1]
    K = np.zeros((n_1, n_2))
    u_num=[np.sum(X_in_1==i) for i in u_list]
    print(u_num)
    ratio=[ sum(u_num)/len(u_num)/i for i in u_num]
    for i in range(len(u_list)):
        c_1 = np.sqrt(np.sum(X_in_1 == u_list[i]))
        c_2 = np.sqrt(np.sum(X_in_2 == u_list[i]))
        ind_1 = np.where(X_in_1 == u_list[i])[1]
        ind_2 = np.where(X_in_2 == u_list[i])[1]
        K[np.ix_(ind_1, ind_2)] = ratio[i] / c_1 / c_2
    return K
